...
def ensure_dependencies_loaded() -> None:
    """
    延迟加载依赖，避免 `--help` 这类纯参数查询也因缺依赖直接退出。
    """
    global bibtexparser, BibTexParser, SemanticScholar, arxiv, requests

    if bibtexparser is None or BibTexParser is None:
        try:
            import bibtexparser as _bibtexparser  # type: ignore
            from bibtexparser.bparser import BibTexParser as _BibTexParser  # type: ignore

            bibtexparser = _bibtexparser
            BibTexParser = _BibTexParser
        except ImportError:
    ...
    if SemanticScholar is None:
        try:
            from semanticscholar import SemanticScholar as _SemanticScholar  # type: ignore
