...
output_pdf_path: full path for the output PDF; if an existing directory is passed, write <main>.pdf there.
"""
import base64
import os
import re
...
_CMD_ALREADY_DEFINED_WITH_PATH_RE = re.compile(
    r"^\./(?P<path>[^:\n]+):(?P<line>\d+):\s+LaTeX Error: Command \\(?P<cmd>[A-Za-z@]+) already defined",
    re.MULTILINE,
)
