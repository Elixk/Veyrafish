...
<a href="https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/one-click-install-release-copy.md">
  <img src="https://img.shields.io/badge/⚡_立即安装-7B61FF?style=for-the-badge" alt="安装">
</a>
&nbsp;
<a href="https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/quick-start.md">
...
> **学术生产力的高速公路**：横跨 PubMed/arXiv 等数十个科研数据库的精准检索、综述矩阵整理、引文管理，从论文起草、修改到同行评审的完整出版物流程，以及面向学术翻译与写作的仓库内 Prompt 资产层。
...
这一能力域最近新增了社区镜像技能 `arxiv-translator`，以及仓库内 Prompt 资产层 `prompt-library`；其中已纳入来自 `awesome-ai-research-writing` 的 17 条学术写作 Prompt。
...
_VibeSkills 不是静态快照，而是在持续演进的能力底座。最近已经规范化纳入这套可直接 fork 的基座的内容包括：_

- **社区 Skill 镜像**：`ui-ux-pro-max`、`ai-search-hub`、`ssh-skill`、`pua`、`arxiv-translator`
- **学术论文翻译工作流**：`arxiv-translator` 提供 arXiv e-print 下载、TeX 检查、翻译导向工作流和 PDF 编译支持
- **仓库内 Prompt 资产面**：`prompt-library` 作为仓库自有本地提示词层存在，无需把每一条 Prompt 都做成独立 Skill
- **首批 Prompt 资产**：来自 `awesome-ai-research-writing` 的 17 条学术写作 Prompt 已按 `translation`、`writing`、`review`、`research` 分类接入 `prompt-library`

如果你准备 fork VibeSkills 做自己的公开仓库，推荐遵循这个模式：工具型上游仓库镜像成 Skill，Prompt 合集统一沉淀到 `prompt-library`。
...
运行 `uninstall.ps1 -HostId <host>` 或 `uninstall.sh --host <host>` 是安装的对称路径，它默认 ledger-first 执行 owned-only 清理，最多只会删除 `.vibeskills/install-ledger.json`、`.vibeskills/host-closure.json`、或已经被 `adapters/*/closure.json` 记录的 Vibe 负载。bundled 运行时包只保留可执行契约；完整治理说明见 canonical repo 中的 [`docs/uninstall-governance.md`](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/uninstall-governance.md)。
...
| | 唯一公开入口 |
|:---:|:---|
| **安装入口** | [⚡ 提示词安装（推荐）](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/one-click-install-release-copy.md) |
| **同一入口内的公开版本** | `全量版本 + 可自定义添加治理` / `仅核心框架 + 可自定义添加治理` |
| **结果** | 在一个地方选择宿主 + 动作 + 版本，然后复制对应提示词 |

</div>

现在的安装面已经改成 registry-driven。`HostId` / `--host` 决定宿主语义，同一个公开入口会按 adapter 落到 `governed`、`preview-guidance` 或 `runtime-core`。`full` 与 `minimal` 仍然是两个不同的产品层级，但现在主要区别在于**内部能力宽度与可选兼容负载**，而不是“显示少量 skills”对“直接暴露几百个 skills”。如果你还不知道自己该走哪条路径，先看 [冷启动宿主矩阵](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/cold-start-install-paths.md) 或 [多宿主命令参考](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/recommended-full-path.md)。
...
### 自定义：接入你自己的 Skill 与 Prompt 资产

- [自定义工作流与 Skill 接入指南](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/custom-workflow-onboarding.md)
- [`prompt-library` 使用说明](./bundled/skills/prompt-library/README.md)
- [`skills管理和安装指南.md`](./skills管理和安装指南.md)

如果你是在 fork 之后继续扩展仓库：完整上游工具型仓库按 Skill 镜像接入；成组的 Prompt 资产则统一沉淀到 `prompt-library`。
...
- 📖 [了解系统架构与理念](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/quick-start.md)
- 📜 [VibeSkills 宣言](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/manifesto.md)

**安装与配置**

- ⚡️ [提示词安装（默认推荐）](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/one-click-install-release-copy.md)
- 🧩 [自定义工作流接入](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/custom-workflow-onboarding.md)
- 📄 [OpenClaw 宿主说明](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/openclaw-path.md)
- 📄 [OpenCode 宿主说明](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/opencode-path.md)
- 📁 [手动复制安装（离线）](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/manual-copy-install.md)
- 🛠 [高级 host / lane 参考](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/install/recommended-full-path.md)
- 🧊 [冷启动与其他环境说明](https://github.com/Elixk/MY-VIBE-SKILLS/blob/main/docs/cold-start-install-paths.md)
...
<div align="center">
  ...
  <br/>
  <sub>Made with ❤️ &nbsp;·&nbsp; <a href="https://github.com/Elixk/MY-VIBE-SKILLS">GitHub</a> &nbsp;·&nbsp; <a href="./README.md">English</a></sub>
</div>
