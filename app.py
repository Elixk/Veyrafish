...
# 进程启动时清理上一次未正常结束的僵尸任务（starting/running → error）
def _cleanup_zombie_tasks() -> None:
    """清理因进程异常退出遗留的僵尸任务。在模块顶层初始化时调用一次。"""
    try:
        store = get_task_store(str(LOG_DIR / 'tasks.db'))
        count = store.cleanup_stale_tasks()
        if count > 0:
            logger.info(f"[TaskStore] 已将 {count} 条僵尸任务标记为 error（进程重启）")
    except Exception as exc:
        logger.warning(f"[TaskStore] 僵尸任务清理失败，忽略继续启动: {exc}")
...
@app.route('/assets/png/<path:filename>')
def serve_template_png_assets(filename: str):
    """Serve UI PNG assets stored under templates/assets/png."""
    assets_dir = Path(app.root_path) / 'templates' / 'assets' / 'png'
    file_path = (assets_dir / filename).resolve()

    # 防止目录穿越：必须仍在 assets_dir 下
    try:
    ...
    return send_file(file_path, mimetype='image/png', conditional=True, max_age=3600)


@app.route('/assets/svg/<path:filename>')
def serve_template_svg_assets(filename: str):
    """Serve legacy SVG paths, with PNG fallbacks for the Phase 2 UI assets."""
    svg_assets_dir = Path(app.root_path) / 'templates' / 'assets' / 'svg_assets'
    svg_file_path = (svg_assets_dir / filename).resolve()

    try:
        svg_file_path.relative_to(svg_assets_dir.resolve())
    except Exception:
        abort(404)

    if svg_file_path.is_file():
        return send_file(svg_file_path, mimetype='image/svg+xml', conditional=True, max_age=3600)

    png_aliases = {
        'header-bamboo-corner.svg': 'bamboo2.png',
        'logo-seal-red.svg': 'seal-trans.png',
        'quote-seal-left.svg': 'seal-trans.png',
        'quote-boat-landscape.svg': 'boat2-cple-trans.png',
        'title-seal-red.svg': 'seal-trans.png',
        'icon-insight-eye.svg': 'eye.png',
        'icon-media-brush.svg': 'pen1.png',
        'icon-query-search.svg': 'infocloud.png',
        'icon-forum-chat.svg': 'circle1.png',
        'icon-report-scroll.svg': 'pen2.png',
        'footer-mountain-left.svg': 'bottom1.png',
        'footer-mountain-right-boat.svg': 'bottom2.png',
        'footer-seal.svg': 'seal-trans.png',
        'cloud-flourish.svg': 'infocloud.png',
    }
    png_filename = png_aliases.get(filename)
    if not png_filename:
        abort(404)

    png_assets_dir = Path(app.root_path) / 'templates' / 'assets' / 'png'
    png_file_path = (png_assets_dir / png_filename).resolve()
    try:
        png_file_path.relative_to(png_assets_dir.resolve())
    except Exception:
        abort(404)
    ...
    return send_file(png_file_path, mimetype='image/png', conditional=True, max_age=3600)
...
def search():
    ...
    running_apps = [
        name for name, info in processes.items()
        if info['status'] == 'running' and name in api_ports
    ]
    ...
    # 向运行中的应用并发发送搜索请求
    results = {}
    with ThreadPoolExecutor(max_workers=min(3, len(running_apps))) as pool:
        futures = {
            pool.submit(_post_search_to_engine, app_name, api_ports[app_name], query): app_name
...
def _post_search_to_engine(app_name: str, api_port: int, query: str) -> dict:
    """向单个子引擎转发搜索请求，供 /api/search 并发调用。"""
    try:
...
def get_evidence(task_id):
    """查询指定任务的共享证据与聚合摘要。"""
    engine = request.args.get('engine') or None
    try:
        min_confidence = float(request.args.get('min_confidence', 0.0))
    except (TypeError, ValueError):
        return jsonify({'success': False, 'message': 'min_confidence 必须是数字'}), 400
    try:
    ...
    try:
        store = get_evidence_store(str(LOG_DIR / 'tasks.db'))
        evidences = store.query_by_task(
            task_id=task_id,
            engine=engine,
        ...
        logger.exception("查询证据失败")
        return jsonify({'success': False, 'message': f'查询证据失败: {exc}'}), 500
...
def start_system():
    """
    接收启动请求后立即返回 task_id，后台线程执行实际启动工作。

    旧行为：同步阻塞 30s+ 等待所有子进程起来再返回。
    新行为：立即返回 {"success": true, "task_id": "...", "status": "starting"}，
            前端可轮询 /api/system/task/<task_id> 获取进度。
    """
    ...
    # 创建任务记录
    store = get_task_store(str(LOG_DIR / 'tasks.db'))
    task_id = store.create_task('system_start', payload={})
    ...
    def _run_in_background():
        try:
            ...
            if success:
                ...
                store.update_task(task_id, 'completed', result={'logs': logs})
                logger.info(f"[TaskStore] 系统启动任务完成: {task_id}")
            else:
                _set_system_state(started=False)
                store.update_task(task_id, 'error', result={'logs': logs, 'errors': errors})
                logger.error(f"[TaskStore] 系统启动任务失败: {task_id}")
        except Exception as exc:  # pragma: no cover
            logger.exception(f"[TaskStore] 系统启动任务异常: {exc}")
            _set_system_state(started=False)
            store.update_task(task_id, 'error', result={'error': str(exc)})
        finally:
            _set_system_state(starting=False)

    t = threading.Thread(target=_run_in_background, daemon=True, name=f'system-start-{task_id[:8]}')
    t.start()
    ...
    return jsonify({
        ...
        'task_id': task_id,
        'status': 'starting',
        'message': '系统启动任务已提交，请轮询 /api/system/task/<task_id> 查询进度',
    })


@app.route('/api/system/task/<task_id>')
def get_system_task(task_id: str):
    """查询单次系统启动任务的状态与结果。"""
    store = get_task_store(str(LOG_DIR / 'tasks.db'))
...
def list_system_tasks():
    """查询最近 20 条系统启动任务记录（跨进程重启持久化）。"""
    store = get_task_store(str(LOG_DIR / 'tasks.db'))
    tasks = store.list_tasks(limit=20, task_type='system_start')
