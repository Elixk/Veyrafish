...
try:
    ...
    get_evidence_store = None  # type: ignore
...
class ReportAgent:
    ...
    def generate_report(self, query: str, reports: List[Any], forum_logs: str = "",
                        ...
                        evidence_task_ids: Optional[Any] = None) -> str:
        """
        ...
        参数:
            ...
            evidence_task_ids: 可选的研究任务 ID 列表/字典，用于读取共享证据层。
        ...
        evidence_context = self._load_evidence_context(evidence_task_ids)
        ...
        if evidence_context.get("total_count", 0) > 0:
            logger.info(f"共享证据已接入: {evidence_context['total_count']} 条")
    ...
    def _build_generation_context(
        ...
        return {
            ...
            "data_bundles": [{"type": "shared_evidence", **evidence_context}]
                if evidence_context and evidence_context.get("total_count", 0) > 0 else [],
    ...
    def _load_evidence_context(self, evidence_task_ids: Optional[Any]) -> Dict[str, Any]:
        """从共享证据层读取 ReportEngine 可消费的聚合上下文。"""
        task_ids: List[str] = []
        if isinstance(evidence_task_ids, dict):
            task_ids = [str(v) for v in evidence_task_ids.values() if v]
        elif isinstance(evidence_task_ids, (list, tuple, set)):
            task_ids = [str(v) for v in evidence_task_ids if v]
        elif isinstance(evidence_task_ids, str) and evidence_task_ids:
            task_ids = [evidence_task_ids]

        if not task_ids or not _EVIDENCE_STORE_AVAILABLE or get_evidence_store is None:
            return {"task_ids": task_ids, "total_count": 0, "aggregates": {}, "top_claims": []}
        ...
        try:
            ...
            for task_id in task_ids:
                ...
                top_claims.extend(aggregate.top_claims)
            return {
                "task_ids": task_ids,
            ...
            logger.warning(f"读取共享证据失败，忽略继续: {exc}")
            return {
                "task_ids": task_ids,
                "total_count": 0,
                "aggregates": {},
